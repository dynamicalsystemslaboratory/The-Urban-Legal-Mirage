...
To create Supplementary Table 3, evaluate the Mathematica notebook `Linear trend analysis of lawyer affordability.nb`. This table is based on `Annual_Average_Affordability_All.csv`, not `Legal_Economy_Results.csv`.
To create Supplementary Figure 7, run `Scripts/Affordability/Lawyers_2024_Spearman_Plot.ipynb`.

After the annual CSVs have been created, the Figure 4, Supplementary Table 3, and Supplementary Figure 7 analyses are independent and may be run in any order.
...
## Source files and scripts for Figure 4 and Supplementary Tables 3–4
...
Figure 4 is reproduced with `Scripts/Affordability/Affordability analysis.nb` using the two annual summary files generated by `Extended_Professions_Affordability.ipynb`:
...
Panel **a** shows the population-weighted average affordability ratio for lawyers and seven comparison occupations. A higher value indicates that the occupation is more affordable relative to the general local wage level. Panel **b** shows the annual Spearman correlation between city population and the affordability ratio. Filled markers indicate statistically significant correlations (`p < 0.05`), while open markers indicate non-significant correlations (`p >= 0.05`). The vertical line marks the omitted 2020 observation because the required one-year ACS population estimate is not used.
...
The notebook fits a separate linear trend in annual population-weighted affordability for each of the eight occupations and reports the trend coefficient, 95% confidence interval, p-value, and number of annual observations.

### Supplementary Table 4: legal-economy coupling

Supplementary Table 4 is based on:

```text
Data/Processed Data/Legal-Economy-Coupling/Legal_Economy_Results.csv
```

which is generated by `Scripts/Legal-Economy-Coupling/Legal_Economy_Results.ipynb`. For each year, the table reports the slope and intercept from the linear model relating lawyers per capita to total lawyer salaries divided by metropolitan GDP, together with 95% confidence intervals, `R^2`, and sample size.
